...
def handle_shit(png_path, dmi_path, name):
    ...
    img = Image.new('RGBA', dmi_file.size)
    ...
    for idx, state in enumerate(global_possible_states):
        ...
        state_y_offset = (idx // size_of_input[1]) * 32
    ...
    ztxt_chunk_data = waiter.extract_ztxt_chunk(dmi_path)
    new_file_path = os.path.splitext(output_file)[0] + ".dmi"
    shutil.copy2(output_file, new_file_path)
    waiter.insert_ztxt_chunk(new_file_path, output_file, ztxt_chunk_data)
...
if __name__ == "__main__":
    ...
    # MCRAMON, CHANGE THIS PATH TO THE PATH OF YOUR PARADISE WALLS FOLDER
    wall_folder = r"C:\Users\Bluecloud\Documents\github\Paradise\icons\turf\walls"
